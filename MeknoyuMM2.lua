-- [[ MEKNOYU GUI | MM2 ]] --
-- UPDATED: GRAB GUN WITH POSITION RETURN & REMOVED PICKUP BUTTON
...
local CloseBtn = Instance.new("TextButton"); CloseBtn.Name = "CloseBtn"; CloseBtn.Size = UDim2.new(0, 30, 0, 30); CloseBtn.Position = UDim2.new(0.93, 0, 0.125, 0); CloseBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50); CloseBtn.Text = "X"; CloseBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CloseBtn.TextSize = 16; CloseBtn.Font = Enum.Font.SourceSansBold; CloseBtn.Parent = Header
Instance.new("UICorner", CloseBtn).CornerRadius = UDim.new(1, 0)

local ButtonContainer = Instance.new("ScrollingFrame")
ButtonContainer.Name = "ButtonContainer"; ButtonContainer.Size = UDim2.new(1, 0, 1, -40); ButtonContainer.Position = UDim2.new(0, 0, 0, 40); ButtonContainer.BackgroundTransparency = 1; ButtonContainer.ScrollBarThickness = 6; ButtonContainer.CanvasSize = UDim2.new(0, 0, 0, 400); ButtonContainer.Parent = MainFrame
...
MiniButton.Name = "KovexMiniButton"; MiniButton.Size = UDim2.new(0, 55, 0, 55); MiniButton.BackgroundColor3 = Color3.fromRGB(45, 45, 50); MiniButton.Text = "Mekno"; MiniButton.TextColor3 = Color3.fromRGB(255, 255, 255); MiniButton.Visible = false; MiniButton.Active = true; MiniButton.Draggable = true; MiniButton.Parent = ScreenGui
...
local function setInvisState(state)
    ...
    if state then
        ...
        task.wait(0.05)
        local scf = r.CFrame; local under = r.Position + Vector3.new(0, -100, 0); c:MoveTo(under)
        task.wait(0.1)
...
local function createFeatureButton(text, layoutOrder, callback)
    ...
    Btn.BackgroundColor3 = Color3.fromRGB(50, 50, 60); Btn.Text = text; Btn.TextColor3 = Color3.fromRGB(255, 255, 255); Btn.TextSize = 13; Btn.Font = Enum.Font.SourceSansBold; Btn.LayoutOrder = layoutOrder; Btn.Parent = ButtonContainer
...
createFeatureButton("Player ESP: OFF", 1, function(btn)
    ...
    btn.Text = EspEnabled and "Player ESP: ON" or "Player ESP: OFF"; btn.BackgroundColor3 = EspEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function()
        while EspEnabled do
...
createFeatureButton("Aimbot Murderer: OFF", 2, function(btn)
    ...
    btn.Text = MurdererAimbotEnabled and "Aimbot Murderer: ON" or "Aimbot Murderer: OFF"; btn.BackgroundColor3 = MurdererAimbotEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function()
...
createFeatureButton("Noclip: OFF", 3, function(btn)
    ...
    btn.Text = NoclipEnabled and "Noclip: ON" or "Noclip: OFF"; btn.BackgroundColor3 = NoclipEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
end)
RunService.Stepped:Connect(function() if NoclipEnabled and LocalPlayer.Character then for _, part in pairs(LocalPlayer.Character:GetDescendants()) do if part:IsA("BasePart") then part.CanCollide = false end end end end)

createFeatureButton("Speed: OFF", 4, function(btn)
    SpeedEnabled = not SpeedEnabled
    btn.Text = SpeedEnabled and "Speed: ON (50)" or "Speed: OFF"; btn.BackgroundColor3 = SpeedEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function() while SpeedEnabled do task.wait(); if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.WalkSpeed = CustomSpeed end end if not SpeedEnabled and LocalPlayer.Character then LocalPlayer.Character.Humanoid.WalkSpeed = 16 end end)
end)
...
createFeatureButton("Gun ESP: OFF", 6, function(btn)
    ...
    btn.Text = GunEspEnabled and "Gun ESP: ON" or "Gun ESP: OFF"; btn.BackgroundColor3 = GunEspEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function()
...
createFeatureButton("Aimbot Sheriff: OFF", 7, function(btn)
    ...
    btn.Text = SheriffAimbotEnabled and "Aimbot Sheriff: ON" or "Aimbot Sheriff: OFF"; btn.BackgroundColor3 = SheriffAimbotEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function()
...
createFeatureButton("Grab Gun: OFF", 8, function(btn)
    ...
    btn.Text = AutoGrabEnabled and "Grab Gun: ON" or "Grab Gun: OFF"; btn.BackgroundColor3 = AutoGrabEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function()
...
createFeatureButton("Invisible: OFF", 9, function(btn)
    InvisibleEnabled = not InvisibleEnabled
    btn.Text = InvisibleEnabled and "Invisible: ON" or "Invisible: OFF"; btn.BackgroundColor3 = InvisibleEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    setInvisState(InvisibleEnabled)
end)

createFeatureButton("Anti Fling: OFF", 10, function(btn)
    AntiFlingEnabled = not AntiFlingEnabled
    btn.Text = AntiFlingEnabled and "Anti Fling: ON" or "Anti Fling: OFF"; btn.BackgroundColor3 = AntiFlingEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
end)
RunService.Stepped:Connect(function() if AntiFlingEnabled and LocalPlayer.Character then for _, part in pairs(LocalPlayer.Character:GetDescendants()) do if part:IsA("BasePart") then part.RotVelocity = Vector3.new(0, 0, 0); part.Velocity = Vector3.new(0,0,0) end end end end)

createFeatureButton("TP Kill Target: OFF", 11, function(btn)
    TpKillEnabled = not TpKillEnabled
    btn.Text = TpKillEnabled and "TP Kill Target: ON" or "TP Kill Target: OFF"; btn.BackgroundColor3 = TpKillEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function()
...
createFeatureButton("TP To Murderer: OFF", 12, function(btn)
    ...
    btn.Text = TpToMurdererEnabled and "TP To Murderer: ON" or "TP To Murderer: OFF"; btn.BackgroundColor3 = TpToMurdererEnabled and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 60)
    task.spawn(function()
...
-- ====================================================================
-- [[ 14. AUTO FARM COIN: SEPARATED INSTANT ]]
-- ====================================================================
getgenv().AutoFarmCoinEnabled = false 

local noclipConnection
noclipConnection = RunService.Stepped:Connect(function()
    if getgenv().AutoFarmCoinEnabled and LocalPlayer.Character then
...
createFeatureButton("Auto Farm Coin: OFF", 14, function(btn)
    ...
    if getgenv().AutoFarmCoinEnabled then
        task.spawn(function()
            while getgenv().AutoFarmCoinEnabled do
                task.wait()
...
game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Meknoyu GUI", Text = "System Ready", Duration = 4})
