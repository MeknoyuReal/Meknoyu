...
saveBtn.MouseButton1Click:Connect(function()
    ...
    if states.savepos and hrp then 
        savedPos = hrp.CFrame 
        lastPos = savedPos
    end
end)
...
main.Position = UDim2.new(0.5, -225, 0.5, -150) -- Posisi Tengah
...
-- Sidebar Tab (Disesuaikan Canvas & Layout agar muat tombol baru)
...
local pageGames = createPage("Games") -- Menambahkan halaman Games
...
addTab("Tap Info", pageInfo) -- Diubah namanya menjadi Tap Info sesuai permintaan
addTab("Tab Games", pageGames) -- Menambahkan tombol Tab Games

-- // TAB INFO CONTENT //
-- Menghapus konten teks bawaan lama dan menggantinya dengan teks proteksi kustom Anda
local infoText = Instance.new("TextLabel", pageInfo)
infoText.Size = UDim2.new(0.95, 0, 0, 200)
infoText.Position = UDim2.new(0.025, 0, 0, 5)
...
-- // TAB GAMES CONTENT //
local gamesText = Instance.new("TextLabel", pageGames)
gamesText.Size = UDim2.new(0.95, 0, 0, 50)
gamesText.Position = UDim2.new(0.025, 0, 0, 5)
gamesText.BackgroundTransparency = 1
gamesText.TextColor3 = Color3.fromRGB(200, 200, 200)
gamesText.TextSize = 12
gamesText.Font = Enum.Font.Gotham
gamesText.Text = "No custom games loaded yet. Add your game scripts here!"
...
local function createToggleBtn(name, key, parent)
    ...
    b.MouseButton1Click:Connect(function()
        ...
        if key == "noclip" and not states.noclip then
        ...
        elseif key == "esp" and not states.esp then
            for _, p in pairs(players:GetPlayers()) do if p.Character and p.Character:FindFirstChild("MeknoHighlight") then p.Character.MeknoHighlight:Destroy() end end
        ...
        elseif key == "disco" then
            if states.disco then discoSky.Parent = lighting; skyText.Visible = true else discoSky.Parent = nil; skyText.Visible = false; lighting.Ambient = oldAmbient; lighting.OutdoorAmbient = oldOutdoor; lighting.FogColor = oldFog end
        elseif key == "hcxxr" and not states.hcxxr then
            lighting.Ambient = oldAmbient; lighting.OutdoorAmbient = oldOutdoor
            for _, p in pairs(players:GetPlayers()) do if p.Character then for _, v in pairs(p.Character:GetDescendants()) do if v:IsA("Fire") and v.Name == "MeknoFire" then v:Destroy() end end end end
        elseif key == "addpart" and not states.addpart then
            if platformPart then platformPart:Destroy(); platformPart = nil end
        elseif key == "antirobux" and not states.antirobux then
            pcall(function() guiService.MenuOpened:Disconnect() end)
        elseif key == "xray" and not states.xray then
            for obj, trans in pairs(originalTransparencies) do if obj and obj.Parent then obj.Transparency = trans end end; originalTransparencies = {}
        elseif key == "bypassac" and states.bypassac then
...
local function GodModeSeatNoInvis(state)
    ...
    if state then
        ...
        task.wait(0.05); local scf = r.CFrame; local under = r.Position + Vector3.new(0, -100, 0); c:MoveTo(under); task.wait(0.1)
        local s = Instance.new("Seat", workspace); s.Anchored = true; s.CanCollide = false; s.Transparency = 1; s.Position = under; s.Name = "invischair"
...
godToggleBtn.MouseButton1Click:Connect(function()
    states.god = not states.god; godToggleBtn.Text = "God Mode : " .. (states.god and "ON" or "OFF"); godToggleBtn.BackgroundColor3 = states.god and Color3.fromRGB(50, 100, 50) or Color3.fromRGB(35, 35, 35)
    if states.god then GodModeSeatNoInvis(true)
        godConnection = rs.Heartbeat:Connect(function() if char and hum and hrp then hum.MaxHealth = math.huge; hum.Health = math.huge; hum:SetStateEnabled(Enum.HumanoidStateType.Dead, false) end end)
    else if godConnection then godConnection:Disconnect() end; GodModeSeatNoInvis(false); if hum then hum.MaxHealth = 100; hum.Health = 100; hum:SetStateEnabled(Enum.HumanoidStateType.Dead, true) end end
end)
...
invisToggleBtn.MouseButton1Click:Connect(function()
    states.invis = not states.invis; invisToggleBtn.Text = "Invisible : " .. (states.invis and "ON" or "OFF"); invisToggleBtn.BackgroundColor3 = states.invis and Color3.fromRGB(50, 100, 50) or Color3.fromRGB(35, 35, 35); setInvisState(states.invis)
end)
...
createPlainBtn("Tool TP Logic", pageNoToggle, function() 
    states.tooltp = not states.tooltp
    if states.tooltp then giveTpTool() else local t = plr.Backpack:FindFirstChild("Mekno TP Tool") or (char and char:FindFirstChild("Mekno TP Tool")); if t then t:Destroy() end end 
end)

createPlainBtn("KILL ALL", pageNoToggle, function()
    for i = 1, 10 do for _, p in pairs(players:GetPlayers()) do if p ~= plr and p.Character and p.Character:FindFirstChild("Humanoid") then p.Character.Humanoid.Health = 0; p.Character.Humanoid:TakeDamage(999999) end end; task.wait(0.02) end
end)
...
createPlainBtn("Message Horror", pageNoToggle, function()
    task.spawn(function()
        local oldFrame = Instance.new("Frame", screenGui); oldFrame.Size = UDim2.new(1, 0, 0.25, 0); oldFrame.Position = UDim2.new(0, 0, 0.35, 0); oldFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0); oldFrame.BackgroundTransparency = 0.4
        local oldLabel = Instance.new("TextLabel", oldFrame); oldLabel.Size = UDim2.new(1, 0, 1, 0); oldLabel.BackgroundTransparency = 1; oldLabel.TextColor3 = Color3.fromRGB(255, 255, 255); oldLabel.TextSize = 32; oldLabel.Font = Enum.Font.Legacy
        oldLabel.Text = "Are you stuck here forever?"; task.wait(3.5)
        oldLabel.Text = "You're scared here hahahaha\240\159\152\136"; task.wait(3.5)
        oldLabel.Text = "I'm going to crash this server now hahahaha"; task.wait(3.5); oldFrame:Destroy()
    end)
end)

createPlainBtn("MAN??? (Server Cook)", pageNoToggle, function()
    task.spawn(function() while true do starterGui:SetCore("SendNotification", { Title = "Meknoyu Team", Text = "SERVER ARE COOKED NGAW\240\159\152\136", Duration = 9999 }); task.wait(0.5) end end)
    local topGui = screenGui:FindFirstChild("MeknoTopPermanent") or Instance.new("ScreenGui", screenGui); topGui.Name = "MeknoTopPermanent"; topGui.DisplayOrder = 1000000
    local topFrame = Instance.new("Frame", topGui); topFrame.Size = UDim2.new(0, 500, 0, 35); topFrame.Position = UDim2.new(0.5, -250, 0.02, 0); topFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    local topLabel = Instance.new("TextLabel", topFrame); topLabel.Size = UDim2.new(1, 0, 1, 0); topLabel.BackgroundTransparency = 1; topLabel.Text = "KEBIXEVEBJDDGEBEJDOXI3ERKF9U3V33R8V3B33INXC"; topLabel.TextColor3 = Color3.new(1,1,1); topLabel.TextScaled = true
    task.wait(0.1)
    local loadGui = Instance.new("ScreenGui", screenGui); loadGui.DisplayOrder = 999999; local loadFrame = Instance.new("Frame", loadGui); loadFrame.Size = UDim2.new(1,0,1,0)
    local cookLabel = Instance.new("TextLabel", loadFrame); cookLabel.Size = UDim2.new(1, 0, 0.3, 0); cookLabel.Position = UDim2.new(0, 0, 0.35, 0); cookLabel.BackgroundTransparency = 1; cookLabel.Text = "YOUXXXXXXXAREXXXXXCOOKKED\240\159\152\136XXXX"; cookLabel.TextColor3 = Color3.new(0,0,0); cookLabel.TextSize = 42
    local cn = rs.RenderStepped:Connect(function() loadFrame.BackgroundColor3 = Color3.fromHSV((tick() * 0.5) % 1, 0.8, 1) end); task.wait(5); cn:Disconnect(); loadGui:Destroy()
    for _, obj in pairs(workspace:GetChildren()) do if not obj:IsA("Camera") and not obj:IsA("Terrain") and not players:GetPlayerFromCharacter(obj) then if obj:IsA("BasePart") then obj.Transparency = 1; obj.CanCollide = false elseif obj:IsA("Model") or obj:IsA("Folder") then for _, child in pairs(obj:GetDescendants()) do if child:IsA("BasePart") then child.Transparency = 1; child.CanCollide = false end end end end end
    local bp = workspace:FindFirstChild("MeknoLocalBaseplate") or Instance.new("Part", workspace); bp.Name = "MeknoLocalBaseplate"; bp.Size = Vector3.new(2000, 4, 2000); bp.Position = Vector3.new(0, -2, 0); bp.Anchored = true; bp.CanCollide = true
    if hrp then lastPos = CFrame.new(0, 5, 0); hrp.CFrame = lastPos end
    local horrorTracks = {1837874627, 1836740615, 1843358057, 1839019688, 1841348122}
    for _, audioId in pairs(horrorTracks) do task.spawn(function() local sound = Instance.new("Sound", workspace); sound.SoundId = "rbxassetid://" .. audioId; sound.Volume = 6; sound.Looped = true; sound:Play() end) end
    local rainPart = Instance.new("Part", workspace); rainPart.Size = Vector3.new(2000, 1, 2000); rainPart.Position = Vector3.new(0, 150, 0); rainPart.Anchored = true; rainPart.Transparency = 1; rainPart.CanCollide = false
    local attachment = Instance.new("Attachment", rainPart); local bloodRain = Instance.new("ParticleEmitter", attachment); bloodRain.Texture = "rbxassetid://242336336"; bloodRain.Rate = 50000; bloodRain.Speed = NumberRange.new(120, 180); bloodRain.Lifetime = NumberRange.new(3, 4)
end)
...
rs.RenderStepped:Connect(function()
    ...
    if states.antirag or states.antifling then hum:SetStateEnabled(Enum.HumanoidStateType.Ragdoll, false); hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false)
        if hrp then if states.antifling then hrp.Velocity = Vector3.new(0, 0, 0); hrp.RotVelocity = Vector3.new(0, 0, 0); for _, part in pairs(char:GetDescendants()) do if part:IsA("BasePart") then part.CanCollide = true end end end
            if hrp.Velocity.Magnitude > 75 or hrp.RotVelocity.Magnitude > 75 then if lastPos and not states.speed then hrp.Velocity = Vector3.new(0, 0, 0); hrp.RotVelocity = Vector3.new(0, 0, 0); hrp.CFrame = lastPos end end
        end
    end
